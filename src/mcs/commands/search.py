"""`mcs search` — hybrid search over brain/."""
...
from mcs.adapters.search import SearchHit, search as core_search
...
def _render_table(hits: list[SearchHit], query: str, elapsed_ms: int) -> None:
    header = f"🔍 [cyan]{query!r}[/cyan]  [dim]({elapsed_ms}ms · {len(hits)} results)[/dim]"
    console.print(f"\n{header}\n")
...
def search_cmd(
    ...
    if as_json:
        typer.echo(json.dumps([h.to_dict() for h in hits], ensure_ascii=False, indent=2))
        return
